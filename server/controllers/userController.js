...
exports.getRestaurants = (req, res, next) => {
	Partner.find()
		...
		.sort({ createdAt: -1 })
		.then((partners) => {
			const partnersFinal = partners.filter((restaurant) => {
				return restaurant.account.isVerified === true;
			});
			res.status(200).json({
				restaurants: partnersFinal,
				totalItems: partnersFinal.length,
			});
		})
		.catch((err) => {
...
exports.getRestaurant = (req, res, next) => {
	...
	Partner.findById(restId)
		.populate("items")
		// .then((partner) => {
		//   return Item.find({ _id: { $in: partner.items } });
		// })
		.then((restaurant) => {
	...
	Account.findById(accountId)
		...
		.then((user) => {
			if (user) {
				...
				return Partner.findOne({ account: accountObj._id })
					.populate("items")
...
exports.postOrder = (req, res, next) => {
	...
	Account.findById(req.loggedInUserId)
		...
		.then((result) => {
			const partners = result.cart.items.reduce((acc, item) => {
			...
			for (let [partner, cartItem] of Object.entries(partners)) {
				Partner.findById(partner).then((partner) => {
					...
					for (const clientId of Object.keys(app.clients)) {
						...
						if (clientId.toString() === partner._id.toString()) {
	...
	Account.findById(accountId)
		.then((account) => {
			if (account.role === "ROLE_USER")
				return User.findOne({ account: account._id });
			if (account.role === "ROLE_PARTNER")
				return Partner.findOne({ account: account._id });
		})
		.then((result) => {
			...
			if (result instanceof Partner)
				return Order.find({ "partner.partnerId": result._id }).sort({
					createdAt: -1,
				});
		})
		.then((orders) => {
...
exports.getRestaurantsByAddress = (req, res, next) => {
	...
	Partner.find()
		...
		.sort({ createdAt: -1 })
		.then((partners) => {
			const partnersVerified = partners.filter((restaurant) => {
				return restaurant.account.isVerified === true;
			});

			const partnersFinal = partnersVerified.reduce((result, partner) => {
				const lat2 = partner.address.lat;
				const lon2 = partner.address.lng;
				...
				const d = R * c; // in km
				if (d < 10) result.push(partner);

				return result;
			}, []);

			return partnersFinal;
		})
		.then((results) => {
			res.status(200).json({
				restaurants: results,
