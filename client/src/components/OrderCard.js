		...
		>
			<div style={{ marginLeft: 20 }}>
				...
				<Typography gutterBottom variant="body1" color="textPrimary">
					{role === "ROLE_USER" && `Ordered From - ${order.partner.name}`}
					{role === "ROLE_PARTNER" &&
						`Ordered By - ${order.user.name}, +91 ${order.user.address.phoneNo}`}
				</Typography>
				{role === "ROLE_USER" && (
					<Typography gutterBottom variant="body1" color="textPrimary">
						Call - +91 {order.partner.phone}
					</Typography>
				)}
				...
				{role === "ROLE_PARTNER" && order.status === "Placed" && (
					<>
				...
				{role === "ROLE_PARTNER" && order.status === "Accepted" && (
					<Button className={classes.buttonAccept} onClick={handleDelivery}>
						Out For Delivery
					</Button>
				)}
				{role === "ROLE_PARTNER" && order.status === "Out For Delivery" && (
